...
template <typename T>
int _partitionV1(T* pData, int left, int right)
{
...
// improved version2
template<typename T>
void _quickSortV2(T * pData, int left, int right)
{
	const int INSERTION_SORT_THRESHOLD = 32;
	if( right - left <= INSERTION_SORT_THRESHOLD )
	{
    ...
    _quickSortV2(pData, left, pilot - 1);
    _quickSortV2(pData, pilot + 1, right);
}

// improved version2
template<typename T>
void quickSortV2(T * pData, int length)
{
	srand(time(NULL));
	_quickSortV2(pData, 0, length - 1);
}
...
{	
	...
	// recursion
    if (left < j)
		_quickSortV1(pData, left, j);
    if (i < right)
		_quickSortV1(pData, i, right);
}
