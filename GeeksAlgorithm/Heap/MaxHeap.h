#ifndef GEEKS_MAX_HEAP_H
#define GEEKS_MAX_HEAP_H
...
private:
	...
	void shiftUp(int k)
	{
		while(k > 1 && m_pData[k / 2] < m_pData[k])
		{
	...
	void shiftDown(int k)
	{
		while(2 * k <= m_count)
		{
            ...
            if(j + 1 <= m_count && m_pData[j + 1] > m_pData[j])
                ++j;
            // now, m_pData[j] is the max in m_pData[2*k] and m_pData[2*k+1]
