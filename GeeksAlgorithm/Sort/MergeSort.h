#ifndef GEEKS_SORT_MERGE_H
#define GEEKS_SORT_MERGE_H
...
{
	// for samll size data, use insertion sort is better
...
template<typename T>
void _mergeSortV0(T* pData, int left, int right)
{
	if(left < right)
	{
		int middle = (left + right) / 2;
...
// initial version
template<typename T>
void mergeSortV0(T* pData, int length)
{
	_mergeSortV0(pData, 0, length - 1);
}

#endif //GEEKS_SORT_MERGE_H